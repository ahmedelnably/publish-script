...
# cd to any directory, but will be restored
def preparePackage(version, outFolder):
    toRestore = os.getcwd() # TODO decorator
    ...
    subprocess.run(["gzip", "-9", "-n", os.path.join(document,"changelog.Debian")])
...
def publishArtifact(src, dest, version):
    ...
    subprocess.run(["fakeroot","dpkg-deb", "--build", os.path.join(src,name), os.path.join(dest,name+".deb")])
