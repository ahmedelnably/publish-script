...
root = f"{outputDir}/{packageName}_{version}-1"
usr = f"{root}/usr"
usrlib = f"{usr}/lib"
...
os.chdir("../..")
# strip sharedobjects
subprocess.run("find * -type f | xargs strip --strip-unneeded", shell=True)
...
formattedString = stringData.format(VERSION=version,DATETIME=t)
...
with open("../../control_template") as f:
    stringData = f.read()
formattedString = stringData.format(VERSION=version)
...
subprocess.run(f"fakeroot dpkg-deb --build {packageName}_{version}-1", shell=True)